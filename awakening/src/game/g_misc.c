...
{
	team_t flagtype = TEAM_NONE;

	// Flags are important
	if (strcmp (self->classname, "item_flag_team1") == 0)
		flagtype = TEAM_RED;
	else if (strcmp (self->classname, "item_flag_team2") == 0)
		flagtype = TEAM_BLUE;
	if (flagtype != TEAM_NONE)
	{
		CTFResetFlag (flagtype); // this will free self!
		gi.bprintf (PRINT_HIGH, "The %s flag has returned!\n",
			CTF_TeamName (flagtype));
		return;
	}
...
void SP_light (edict_t *self)
{
	...
	if (!self->targetname || gametype->value > GT_COOP)
	{
		G_FreeEdict (self);
		return;
	}
...
void target_string_use (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	for (e = self->groupmaster; e; e = e->groupchain)
	{
		if (!e->count)
			continue;
		n = e->count - 1;
