...
void Move_Calc (edict_t *ent, vec3_t dest, void(*func)(edict_t*))
{
	...
	{
		if (level.current_entity == ((ent->flags & FL_GROUPSLAVE) ? ent->groupmaster : ent))
		{
			Move_Begin (ent);
		}
		else
...
void button_fire (edict_t *self)
{
	...
	if (self->moveinfo.sound_start && !(self->flags & FL_GROUPSLAVE))
...
void door_use (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	if (self->spawnflags & DOOR_TOGGLE)
	{
		if (self->moveinfo.state == STATE_UP || self->moveinfo.state == STATE_TOP)
		{
			...
			for (ent = self ; ent ; ent = ent->groupchain)
			{
				ent->message = NULL;
				ent->touch = NULL;
				door_go_down (ent);
...
{
	// The players must open the door by themselves with "+use"
	if (other->client)
		return;
...
void Think_CalcMoveSpeed (edict_t *self)
{
	...
	for (ent = self->groupchain; ent; ent = ent->groupchain)
	{
...
void Think_SpawnDoorTrigger (edict_t *ent)
{
	...
	if (ent->flags & FL_GROUPSLAVE)
		return;		// only the group leader spawns a trigger
...
void door_killed (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	...
	door_use (self->groupmaster, attacker, attacker);
}
...
void SP_func_door (edict_t *ent)
{
	...
	// to simplify logic elsewhere, make non-grouped doors into a group of one
	if (!ent->group)
		ent->groupmaster = ent;
