...
{
	// TODO: reimplement it with the new team system
...
void Cmd_Give_f (edict_t *ent)
{
	...
	if (gametype->value != GT_SINGLE_PLAYER && !sv_cheats->value)
	{
...
void Cmd_Inven_f (edict_t *ent)
{
	...
	if (gametype->value != GT_SINGLE_PLAYER && cl->resp.team == TEAM_NONE)
	{
		CTFOpenJoinMenu(ent);
		return;
	}
...
void Cmd_Team_f (edict_t *ent)
{
	...
	if (!*t)
	{
		gi.cprintf (ent, PRINT_HIGH, "You are on the %s team.\n",
					CTF_TeamName (ent->client->resp.team));
		return;
	}
